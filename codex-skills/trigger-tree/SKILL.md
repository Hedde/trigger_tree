...
- Instructions: run `TT_PROJECT_DIR="$PWD" python3 "$PLUGIN/scripts/tt-instructions.py"` and pass through `--init`, `--explain ID`, `--json`, `--check`, or `--min-rate N` when requested. Return its output verbatim. Never add or apply proposed probes without explicit user confirmation; unobserved means missing evidence, not a violation.
